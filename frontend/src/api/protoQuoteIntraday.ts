...
/** 拼 URL 的逻辑单独导出一份：DebugCachePanel.vue 的灰度对比面板要用同一个
 *  URL 发起裸 fetch 做字节数/耗时测量，不应该在两个地方各写一遍容易走样。 */
export function quoteIntradayProtoUrl(code: string): string {
  return `/api/stocks/${code}/quote-intraday.pb`
}

export interface DecodedQuoteIntradayResponse {
  code: number
  message: string
...
/**
 * 只做「二进制 -> 解码后的 Proto 对象」这一步，不做后续的定点数/差值还原。
 * 单独导出是因为 DebugCachePanel.vue 的灰度对比面板需要先用
 * utils/protocolBench.ts 量出真实传输字节数（发起裸 fetch 拿到 ArrayBuffer），
 * 再拿同一份 buffer 复用这里的解码逻辑验证"确实解出了合法数据"——
 * 不应该为了验证正确性再多发一次网络请求。
 */
export function decodeQuoteIntradayResponse(buffer: ArrayBuffer): DecodedQuoteIntradayResponse {
  return QuoteIntradayResponseType.decode(new Uint8Array(buffer)) as unknown as DecodedQuoteIntradayResponse
}
...
export interface RawIntradayPointDelta {
  minuteOffset: number
  priceDelta: unknown
...
export interface RawQuoteIntradayVO {
  code: string
  name: string
  price: unknown
  preClose: unknown
...
export async function getQuoteIntradayProto(
  ...
  const resp = await fetch(quoteIntradayProtoUrl(code), {
    method: 'GET',
  ...
  const decoded = decodeQuoteIntradayResponse(buffer)
